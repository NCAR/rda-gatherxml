...
struct BoxFlags {
  ...
  void initialize(size_t dim_x, size_t dim_y, unsigned char n_pole, unsigned
      char s_pole) {
    clear();
...
extern void create_non_cmd_file_list_cache(std::string file_type, std::
    ...
    string user);
extern void grids_per(size_t nsteps, DateTime start, DateTime end, double&
    gridsper, std::string& unit);
...
extern void summarize_frequencies(std::string caller, std::string user, std::
    string mss_ID_code = "");
extern void summarize_grid_levels(std::string database, std::string caller,
    std::string user);
extern void summarize_grid_resolutions(std::string caller, std::string user,
...
struct ClassificationEntry {
  ClassificationEntry() : key(), src(), start_datetime(), end_datetime(),
      start_lat(0.), start_lon(0.), end_lat(0.), end_lon(0.), min_lat(0.),
      min_lon(0.), max_lat(0.), max_lon(0.), min_pres(0.), max_pres(0.),
      min_speed(0.), max_speed(0.), nfixes(0), pres_units(), wind_units() { }
...
struct DataTypeEntry {
  struct Data {
    ...
    struct VerticalData {
	VerticalData() : min_altitude(1.e38), max_altitude(-1.e38), avg_res(0.),
            avg_nlev(0), res_cnt(0), units() { }
  ...
  void fill_vertical_resolution_data(std::vector<double>& level_list, std::
      string z_positive_direction, std::string z_units);
...
struct IDEntry {
  struct Data {
    Data() : S_lat(), N_lat(), W_lon(), E_lon(), min_lon_bitmap(nullptr),
        max_lon_bitmap(nullptr), start(), end(), nsteps(), data_types_table()
        { }
...
struct ObservationData {
  ObservationData();
  bool added_to_ids(std::string observation_type, IDEntry& ientry, std::string
      data_type, std::string data_type_map, float lat, float lon, double
      unique_timestamp, DateTime *start_datetime, DateTime *end_datetime =
      nullptr);
  bool added_to_platforms(std::string observation_type, std::string
      platform_type, float lat, float lon);
  void set_track_unique_observations(bool track) { track_unique_observations =
      track; }
...
struct ScanLine {
  ScanLine() : date_time(), first_coordinate(), last_coordinate(), subpoint(),
      res(0.), width(0.), subpoint_resolution(0.) { }
...
struct ScanLineEntry {
  ScanLineEntry() : key(), sat_ID(), start_date_time(), end_date_time(),
      scan_line_list() { }
...
extern void write(my::map<ScanLineEntry>& scan_line_table, std::list<std::
    string>& scan_line_table_keys, my::map<ImageEntry>& image_table, std::list<
    std::string>& image_table_keys, std::string caller, std::string user);

} // end namespace gatherxml::markup::SatML

extern void write_finalize(std::string filename, std::string ext, std::string
    tdir_name, std::ofstream& ofs, std::string caller, std::string user);
extern void write_initialize(std::string& filename, std::string ext, std::string
    tdir_name, std::ofstream& ofs, std::string caller, std::string user);
