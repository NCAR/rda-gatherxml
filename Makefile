...
ifeq ($(OKAY_TO_MAKE),1)
	$(RDADATARUN) $(COMPILER) -shared -o $(LIBDIR)/$@.$(NEW_VERSION) -Wl,-soname,$@.$(NEW_VERSION) $(GATHERXMLOBJS)
	$(RDADATARUN) rm -f $(LIBDIR)/$@
	$(RDADATARUN) ln -s $(LIBDIR)/$@.$(NEW_VERSION) $(LIBDIR)/$@
...
ifeq ($(OKAY_TO_MAKE),1)
	...
	$(MAKE) NEW_VERSION=$(NEW_VERSION) TARGET=$@ install
endif
#
...
ifeq ($(strip $(VERSION)),MAJOR)
	...
	if [ $(OKAYTOBUILD) -eq 1 ]; then \
	  make install VERSION=$(THISVERSION) TARGET=$@; \
	fi;
endif
endif
#
...
ifneq ($(or $(findstring BUG,$(VERSION)),$(findstring MINOR,$(VERSION)),$(findstring MAJOR,$(VERSION))),)
	$(RDADATARUN) cp ./$(BUILDDIR)/$(TARGET).$(NEW_VERSION) $(BINDIR)/$(TARGET).$(NEW_VERSION)
	$(RDADATARUN) chmod 740 $(BINDIR)/$(TARGET).$(NEW_VERSION)
	$(RDADATARUN) rm -f $(BINDIR)/$(TARGET)
	$(RDADATARUN) ln -s $(BINDIR)/$(TARGET).$(NEW_VERSION) $(BINDIR)/$(TARGET)
endif
#
