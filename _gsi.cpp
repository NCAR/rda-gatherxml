...
void generate_graphics(MySQL::LocalQuery& query,std::string type,std::string table,std::string gindex)
{
  ...
  while (query.fetch_row(row)) {
    if (gindex.empty()) {
	if (type == "obs") {
	  tnc[nn].query.set("select distinct box1d_row,box1d_bitmap from "+table+" where dsid = '"+meta_args.dsnum+"' and observationType_code = "+row[0]+" and platformType_code = "+row[1]+" order by box1d_row");
	  tnc[nn].imagetag="mss_"+row[2]+"."+row[3];
	}
	else if (type == "fix") {
	  tnc[nn].query.set("select box1d_row,box1d_bitmap from "+table+" where dsid = '"+meta_args.dsnum+"' and classification_code = "+row[0]+" order by box1d_row");
	  tnc[nn].imagetag="mss_"+row[1];
	}
    }
    else {
	if (type == "obs") {
	  tnc[nn].query.set("select distinct box1d_row,box1d_bitmap from ObML.ds"+dsnum2+"_primaries as p left join dssdb.mssfile as x on (x.dsid = 'ds"+meta_args.dsnum+"' and x.mssfile = p.mssID) left join "+table+" as t on t.mssID_code = p.code where x.gindex = "+gindex+" and observationType_code = "+row[0]+" and platformType_code = "+row[1]+" order by box1d_row");
	  tnc[nn].imagetag="mss_gindex_"+gindex+"_"+row[2]+"."+row[3];
	}
	else if (type == "fix") {
	  tnc[nn].query.set("select box1d_row,box1d_bitmap from FixML.ds"+dsnum2+"_primaries as p left join dssdb.mssfile as x on x.mssfile = p.mssID left join "+table+" as t on t.mssID_code = p.code where x.gindex = "+gindex+" and classification_code = "+row[0]+" order by box1d_row");
	  tnc[nn].imagetag="mss_gindex_"+gindex+"_"+row[1];
	}
    }
...
int main(int argc,char **argv)
{
  ...
  if (table_exists(server,"ObML.ds"+dsnum2+"_dataTypes2")) {
    if (gindex.empty()) {
	query.set("select distinct l.observationType_code,l.platformType_code,o.obsType,pf.platformType from ObML.ds"+dsnum2+"_dataTypes2 as d left join ObML.ds"+dsnum2+"_dataTypesList as l on l.code = d.dataType_code left join ObML.obsTypes as o on l.observationType_code = o.code left join ObML.platformTypes as pf on l.platformType_code = pf.code");
	table="search.obs_data";
    }
    else {
	query.set("select distinct l.observationType_code,l.platformType_code,o.obsType,pf.platformType from ObML.ds"+dsnum2+"_primaries as p left join dssdb.mssfile as x on x.mssfile = p.mssID left join ObML.ds"+dsnum2+"_dataTypes2 as d on d.mssID_code = p.code left join ObML.ds"+dsnum2+"_dataTypesList as l on l.code = d.dataType_code left join ObML.obsTypes as o on l.observationType_code = o.code left join ObML.platformTypes as pf on l.platformType_code = pf.code where x.gindex = "+gindex);
  ...
  if (table_exists(server,"FixML.ds"+dsnum2+"_locations")) {
    if (gindex.empty()) {
	query.set("select distinct d.classification_code,c.classification from search.fix_data as d left join FixML.classifications as c on d.classification_code = c.code where dsid = '"+meta_args.dsnum+"'");
	table="search.fix_data";
    }
    else {
	query.set("select distinct d.classification_code,c.classification from FixML.ds"+dsnum2+"_primaries as p left join dssdb.mssfile as x on x.mssfile = p.mssID left join FixML.ds"+dsnum2+"_locations as d on d.mssID_code = p.code left join FixML.classifications as c on d.classification_code = c.code where x.gindex = "+gindex);
