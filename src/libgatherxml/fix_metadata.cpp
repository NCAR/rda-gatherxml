...
void summarize_fix_data(string caller, string user) {
  ...
  if (formats_query.submit(server) < 0) {
    log_error(THIS_FUNC + "(): " + formats_query.error(), caller, user);
  }
  ...
  string error;
  if (server.command("lock tables WFixML.ds" + dsnum2 + "_locations write",
      error) < 0) {
    log_error(THIS_FUNC + "(): " + server.error(), caller, user);
  }
  ...
  if (locations_query.submit(server) < 0) {
    log_error(THIS_FUNC + "(): " + locations_query.error(), caller, user);
  }
  ...
  for (const auto& location_row : locations_query) {
    if (data_file_formats_map.find(location_row[0]) == data_file_formats_map.
      ...
      log_error(THIS_FUNC + "() found a webID (" + location_row[0] + ") in "
          "WFixML.ds" + dsnum2 + "_locations that doesn't exist in WFixML.ds" +
          dsnum2 + "_webfiles2", caller, user);
    }
    SummaryEntry se;
  ...
  for (const auto& key : summary_table.keys()) {
    ...
    if (server.insert("search.fix_data", "'" + metautils::args.dsnum + "', " +
        parts[0] + ", " + parts[1] + ", '" + se.start_date + "', '" + se.
        end_date + "', " + parts[2] + ", '" + se.box1d_bitmap + "'") < 0) {
      error = server.error();
      if (error.find("Duplicate entry") != 0) {
        log_error(THIS_FUNC + "(): " + error, caller, user);
      }
    }
  }
  if (server.command("unlock tables", error) < 0) {
  ...
  if (!error.empty()) {
    log_error(THIS_FUNC + "(): summarize_locations() returned '" + error + "'",
        caller, user);
  }
  server.disconnect();
}
